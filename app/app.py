...
if menu == "О разработчике":
    ...
    **Признаки:**
    ...
    if mode == "Ручной ввод":
        ...
        values['Humidity[%]'] = st.number_input("Влажность (%)", min_value=0.0)
        values['TVOC[ppb]'] = st.number_input("TVOC (ppb)", min_value=0)
        values['eCO2[ppm]'] = st.number_input("eCO₂ (ppm)", min_value=0)
        values['Raw H2'] = st.number_input("Raw H2", min_value=0)
        values['Raw Ethanol'] = st.number_input("Raw Ethanol", min_value=0)
        values['Pressure[hPa]'] = st.number_input("Давление (hPa)", min_value=0.0)
        values['PM1.0'] = st.number_input("PM1.0", min_value=0.0)
        values['PM2.5'] = st.number_input("PM2.5", min_value=0.0)
        values['NC0.5'] = st.number_input("NC0.5", min_value=0)
        values['NC1.0'] = st.number_input("NC1.0", min_value=0)
        values['NC2.5'] = st.number_input("NC2.5", min_value=0)
        values['CNT'] = st.number_input("CNT", min_value=0)
